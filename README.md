...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-apptopology-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudAppTopologyV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAppTopologyV1 <target-name> --package swift-google-cloud-apptopology-v1
```
